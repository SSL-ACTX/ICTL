...
use ictl_core::value::{Arena, EntropicState, MemoryError, Payload, PendingPromise};
use ictl_core::{
    ...
    match &stmt.stmt {
                        ...
                        .map(|p| {
                            (
                                ...
                                p.typ
                                    .as_ref()
                                    .map(|t| {
                                        ictl_core::types::Type::from_typename(t)
                                    })
        ...
        Statement::Assignment {
            ...
            let type_name = var_type.as_ref().and_then(|t| match t {
                ictl_core::TypeName::Custom(name) => Some(name.clone()),
                _ => None,
            });
            ...
            } = expr
            {
                ...
                let meta = ictl_core::value::ValueMetadata {
        ...
        Statement::For {
            ...
            let source_payload = {
                ...
                match mode {
                    ...
                    ictl_core::ForMode::Clone => branch
                        .arena
                        .peek(source)
