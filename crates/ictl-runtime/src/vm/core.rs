...
use ictl_core::value::{Arena, EntropicState, MemoryError, Payload};
use ictl_core::{
...
impl Vm {
    ...
    pub fn execute_statement(
        &mut self,
        branch_id: &str,
        stmt: &ictl_core::SpannedStatement,
                ...
                for (name, meta) in &branch.arena.metadata {
                    if let Some(decay_limit) = meta.decay_after_ms {
                        if branch.local_clock > meta.instantiated_at + decay_limit {
                            ...
                            if let Some(ictl_core::value::EntropicState::Valid(_)) =
        ...
        for branch_name in &branches {
            ...
            for (key, state) in &branch.arena.bindings {
                if let Some(existing) = merged_data.get(key) {
                    ...
                    merged_data.insert(key.to_string(), resolved);
                    if pending_reversion.is_none() {
                        pending_reversion = rev;
                    }
                } else {
                    merged_data.insert(key.to_string(), state.clone());
                }
            }
        }
            ...
            match event {
                crate::vm::state::CausalEvent::ChannelSend {
                    ...
                    let was_received = self.causal_history.iter().skip(i + 1).any(|e| {
                        matches!(e, crate::vm::state::CausalEvent::ChannelRecv { channel_id: c_id, message, .. }
    ...
    pub(crate) fn simulate_branch(
        &mut self,
        branch_id: &str,
        statements: &[ictl_core::SpannedStatement],
