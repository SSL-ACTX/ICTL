...
pub(crate) fn analyze_statement(
    ...
    match &stmt.stmt {
        ...
        Statement::If {
            ...
            let condition_type =
                crate::expression::infer_expression_type(analyzer, condition)?;
        ...
        Statement::TypeDecl {
            ...
            let type_struct = Type::Struct(ictl_core::types::StructType {
                fields: schema,
        ...
        Statement::LoopTick { body } => {
            ...
            let body_cost = crate::statement::estimate_block_cost(analyzer, body);
        ...
        Statement::Assignment {
            ...
            let inferred_target_type = if let Some(expected_type) = var_type {
                ictl_core::types::Type::from_typename(expected_type)
            } else {
                analyzer
            ...
            if let ictl_core::types::Type::Struct(ref s) = expected_type {
        ...
        Statement::Select {
            ...
            for case in cases {
                let case_type =
                    crate::expression::infer_expression_type(analyzer, &case.source)
                        .unwrap_or(ictl_core::types::Type::Unknown);
        ...
        Statement::MatchEntropy {
            ...
            if let Some((binding, branch_body)) = valid_branch {
                ...
                let case_type =
                    crate::expression::infer_expression_type(analyzer, target)?;
